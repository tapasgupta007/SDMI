...
class Attention(nn.Module):
    def __init__(self, hidden_dim):
        super().__init__()
        self.attn = nn.Linear(hidden_dim * 2, 1)  # bi-directional

    def forward(self, rnn_output):
        weights = self.attn(rnn_output).squeeze(-1)
...
def evaluate_model(model, X_test, y_test, scaler_y):
    ...
    print(f"\n📊 BiGRU + Attention Model Performance:\nMSE: {mse:.4f} | MAE: {mae:.4f} | R²: {r2:.4f}")
    return r2
...
model = BiGRUWithAttention(input_dim, hidden_dim).to(DEVICE)
